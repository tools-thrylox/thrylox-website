...
export function deriveRobotState({section='hero',progress=0,mobile=false,reducedMotion=false,event='scroll',ctaActive=false,width=1440,height=900}) {
  ...
  const size=mobile?Math.min(132,Math.max(108,width*.34)):Math.min(390,width*.28);
  const startX=mobile?width-size-20:width*.72;
  const startY=mobile?20:height*.30;
  return {
    section,progress:p,event,mobile,reducedMotion,size,
    x:startX+(reducedMotion?0:depth*(mobile?-12:-width*.09)),
    y:startY+(reducedMotion?0:depth*(mobile?12:height*.17)),
